...
export default function MenuItem({ item }: { item: MenuItem }) {
    ...
    return (
        <div className="flex w-full flex-col items-center justify-between gap-4 bg-itembackground p-4 shadow-lg">
                ...
                {cartItems.find((cart) => cart._idMenuItem === item._id)?._idMenuItem === item._id ?
                    item && (
                        <MenuChoosing item={item}>
                            <Button variant="secondary" className="bg-menusecondary px-8 text-lg font-bold text-menuforeground hover:bg-menuprimary  rounded-none">
                                Add
                            </Button>
                        </MenuChoosing>
                    )
