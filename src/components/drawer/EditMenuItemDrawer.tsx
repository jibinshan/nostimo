...
const EditMenuItemDrawer: FC<MenuItemPopupProps> = ({
  children,
  item,
  index,
}) => {
    ...
    if (item) {
      ...
      if (item.modifiers) {
        ...
        const multiselect = items
          .find((m) => m.modifiers.some((mod) => !mod.multiSelection))
          ?.modifiers.find((m) => !m.multiSelection);
        if (multiselect) {
      ...
      >
        ...
        >
                  ...
                  <div className="z-40 flex w-full justify-between gap-2">
                    ...
                    <p className="text-lg font-semibold text-menusecondary">
                      {menuitem && menuitem.takeawayPrice.value > 0 ? (
                        <>
                          {getCurrencySymbol(menuitem.takeawayPrice.currency)}{" "}
                          {formattedItemPrice(menuitem.takeawayPrice.value)}
                        </>
                      ) : (
                        <>
          ...
          {menuitem?.modifiers.map((mod, index) => {
            return (
              <div
                className="rounded-xl bg-itembackground px-5 py-6"
                key={index}
              >
                <div className="flex flex-col gap-4 space-y-2">
                  ...
                  <div className="z-40 flex h-full w-full flex-col gap-6">
                    {mod?.multiSelection
                      ...
                      : item &&
                        GetModifiersFromItemId(menuitem, items, index).length >
                          0 && (
                          <div>
                            <RadioGroup
          ...
          <DrawerFooter className="flex w-full flex-row justify-start gap-5">
            ...
            <Button
              ...
              onClick={() => {
                ...
                const modifiers: CartItemModifier[] = selectedModifiers.map(
                  (selectedModifier) => {
                    ...
                    for (let i = 0; i < menuitem.modifiers.length; i++) {
                      ...
                      if (
                        groupItems.some(
                          (groupItem) => groupItem._id === selectedModifier._id,
                        )
                      ) {
                        return {
